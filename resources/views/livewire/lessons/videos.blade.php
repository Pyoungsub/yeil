            ...
            <div x-ref="container" class="swiper w-full overflow-hidden">
                <div class="swiper-wrapper">
                    @foreach($lesson->lesson_videos as $lesson_video)
                        <div class="swiper-slide relative" wire:key="video-{{ $lesson_video->id }}">
                            @auth
                                @if(auth()->user()->admin)
                                    <div class="flex items-cetner gap-2 absolute top-0 right-0 z-20">
                                        <button class="bg-white rounded" wire:click="modifyIdolVideo({{$lesson_video->id}})">
                                        ...
                                        <button class="bg-white rounded" wire:click="deleteIdolVideo({{$lesson_video->id}})">
                                ...
                                @endif
                            @endauth
                            {{--<x-video source="{{ $lesson_video->video_path }}" />--}}
                            <x-rectangle-video source="{{ $lesson_video->video_path }}" />
                            {{--<x-square-video source="{{ $lesson_video->video_path }}" />--}}
                        </div>
                    @endforeach
                </div>
            </div>
